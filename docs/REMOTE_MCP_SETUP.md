...
authentication**. This endpoint registers one focused `queueproof_search` tool plus a tiny
read-only routing guide resource, and is bound to the
...
| `queueproof_search` | Search verified connectorIds or indexed document sourceIds and return a grounded answer, cited claims, citation objects, contradictions, missing information, validation, sanitized evidence, provider coverage, latency, HydraDB call count, and relative retrieval cost |
